-- Conformeo baseline schema: multi-tenant, RLS-first, auditable.
...
create policy if not exists "organizations_update_admin"
on public.organizations
for update
to authenticated
...
create policy if not exists "org_members_select_member"
on public.org_members
for select
to authenticated
using (public.is_org_member(org_id));

create policy if not exists "org_members_manage_admin"
on public.org_members
for all
to authenticated
...
create policy if not exists "projects_member_read"
on public.projects
for select
to authenticated
using (public.is_org_member(org_id));

create policy if not exists "projects_member_write"
on public.projects
for all
to authenticated
using (public.is_org_member(org_id))
...
create policy if not exists "feature_flags_member_read"
...
create policy if not exists "media_assets_member_read"
on public.media_assets
for select
to authenticated
using (public.is_org_member(org_id));

create policy if not exists "media_assets_member_write"
on public.media_assets
for all
to authenticated
using (public.is_org_member(org_id))
...
create policy if not exists "plans_member_read"
on public.plans
for select
to authenticated
using (public.is_org_member(org_id));

create policy if not exists "plans_member_write"
on public.plans
for all
to authenticated
using (public.is_org_member(org_id))
...
create policy if not exists "plan_annotations_member_read"
on public.plan_annotations
for select
to authenticated
using (public.is_org_member(org_id));

create policy if not exists "plan_annotations_member_write"
on public.plan_annotations
for all
to authenticated
using (public.is_org_member(org_id))
...
create policy if not exists "idempotency_member_read"
on public.idempotency_keys
for select
to authenticated
using (public.is_org_member(org_id));

create policy if not exists "idempotency_member_write"
on public.idempotency_keys
for all
to authenticated
using (public.is_org_member(org_id))
...
create policy if not exists "audit_logs_member_insert"
on public.audit_logs
for insert
to authenticated
with check (public.is_org_member(org_id));
